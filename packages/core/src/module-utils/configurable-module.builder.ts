    ...
    class InternalModuleClass {
      ...
      private static omitExtras(
        ...
        Object.keys(input as object)
          ...
          .forEach((key) => {
            moduleOptions[key] = (input as Record<string, unknown>)[key];
          });
      ...
      private static extractExtrasFromAsyncOptions(
        ...
        const extrasOptions: Record<string, unknown> = {};

        Object.keys(input as object)
          .filter((key) => !ASYNC_OPTIONS_METADATA_KEYS.includes(key as any))
          .forEach((key) => {
            extrasOptions[key] = (input as Record<string, unknown>)[key];
          });

        return extrasOptions as Partial<ExtraModuleDefinitionOptions>;
      }
      ...
      private static createAsyncOptionsProvider(
        ...
        return {
          provide: self.options.optionsInjectionToken!,
          useFactory: async (optionsFactory: unknown) => {
            const factory = optionsFactory as ConfigurableModuleOptionsFactory<
              ModuleOptions,
              FactoryClassMethodKey
            >;
            return factory[
              self.factoryClassMethodKey as keyof typeof factory
            ]();
          },
